...
function LoadingModal({}) {
    ...
    const getTimeRemaining = () => {
        if (totalEstimatedTime) {
            const minutes = Math.floor(((totalEstimatedTime - (totalEstimatedTime % 60)) / 60) % 60);
    ...
    return (<div className={`${classes.modalBackground} baseFlex`}>
        ...
        <div className={classes.parentLoader}>
          <div className={classes.loader}>
            <svg className={classes.circle} viewBox="25 25 50 50">
              <circle className={classes.loaderPath} cx="50" cy="50" r="20" fill="none" stroke="#70c542" strokeWidth="2"/>
            </svg>
          </div>
        </div>
        {estimatedTimeRemaining && (<div style={{ gap: ".5rem" }} className={"baseVertFlex"}>
